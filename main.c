...
bool IsImageFile(const char *path)
{
    ...
    return TextIsEqual(ext, ".png")  ||
           TextIsEqual(ext, ".jpg")  ||
           TextIsEqual(ext, ".jpeg") ||
           TextIsEqual(ext, ".bmp")  ||
           TextIsEqual(ext, ".tga")  ||
           TextIsEqual(ext, ".gif")  ||
           TextIsEqual(ext, ".qoi");
...
{
  camera.zoom = fminf((float)GetScreenWidth() / texture.width, (float)GetScreenHeight() / texture.height);
  ...
  struct stat pathStat;
    ...
    while (!WindowShouldClose())
    {
        ...
        if (wheel != 0)
        {
            ...
            camera.target = Vector2Add(camera.target, Vector2Subtract(mouseWorldPos, mouseWorldPosAfterZoom));
        }

        if (IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_RIGHT_CONTROL))
        {
            if(IsKeyDown(KEY_EQUAL)) camera.zoom *= 1.1f;
            if(IsKeyDown(KEY_MINUS)) camera.zoom *= 1.0f / 1.1f;
        }
        ...
        if (IsKeyDown(KEY_A) && !IsKeyDown(KEY_LEFT_CONTROL)) camera.target.x -= 10.0f / camera.zoom;
        if (IsKeyDown(KEY_D)) camera.target.x += 10.0f / camera.zoom;

        // Rotation
        if (IsKeyDown(KEY_Q)) camera.rotation--;
        if (IsKeyDown(KEY_E)) camera.rotation++;
        if (IsKeyPressed(KEY_R)) camera.rotation = 0.0f;

        // Flipping
        if (IsKeyPressed(KEY_H)) flipHorizontal = !flipHorizontal;
        if (IsKeyPressed(KEY_V)) flipVertical = !flipVertical;
        ...
        //Showing File info
        if(IsKeyPressed(KEY_I)) info_mode = !info_mode;
        ...
        if (flipHorizontal) sourceRec.width *= -1;
        if (flipVertical) sourceRec.height *= -1;

        Rectangle destRec = { -texture.width / 2.0f, -texture.height / 2.0f, (float)texture.width, (float)texture.height };
